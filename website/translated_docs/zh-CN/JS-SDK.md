...
batch.add(contract.methods.balance(address).call.request({from: 'atx1w9x7ye4qalarnl9v59zzhyn7tug9864rll39n7'}, callback2));
batch.execute();
```
...
*  `address`：String - 要检查余额的账户地址，bech32 address格式，atx开头的为Alaya测试网，atp开头的为Alaya主网
...
`bech32Address` - String: 要检查的bech32格式地址字符串；atx表示测试网地址，atp表示主网地址。
