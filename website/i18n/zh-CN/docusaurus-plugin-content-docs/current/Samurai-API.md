...
## 指导
Samurai是fork以太坊web钱包Metamask为基础进行改造的，以适配PlatON/Alaya网络, 满足基于PlatON/Alaya网络的安全和可用性的需求。使用户可以很方便的管理账户并连接到PlatON/Alaya网络。
...
+ __区块链连接__
  ...
  同时，Samurai可以与任何公开的JSON RPC API与PlatON/Alaya网络的的区块链兼容，包括自定义或私有的区块链网络。
...
```
一旦安装并运行了Samurai，您可以发现新的浏览器页面的开发人员控制台有window.platon对象中可用。您可以通过该对象和Samurai进行交互。
...
+ __熵(Entropy)__
    ...
    + 通常以“熵的位数”或强行获得具有这么大熵的（例如私钥）所需的年数来定义。
...
+ __合约网络__
  ...
  如果您未连接到正确的网络，则不会将任何交易发送到合约，因此请确保您连接正确的网络！
...
+ __合约bytecode__
  ...
  如果您的Web应用程序要发布预先编译的新智能合约，则可能需要包含一些bytecode。在这种情况下，您将不会事先知道合约地址，而必须发布，监视要处理的交易，然后从完成的交易中提取最终合约地址。

  如果从bytecode发布合约，则您还需要ABI与之交互！bytecode未描述如何与最终合约交互。
+ __合约源代码__

  如果您的网站允许用户编辑智能合约源代码并进行编译，您可以导入整个编译器，在这种情况下，您将从该源代码派生您的bytecode和ABI，最终您将从发布该bytecode的已完成交易中获取合约的地址。
### 访问账户

用户账户在PlatON/Alaya的各种网络环境中都可以使用，包括用作身份标识符和签署交易。为了请求用户签名或让用户批准交易，必须能够访问用户的帐户。在wallet methods下面涉及的签名或交易的批准，所有都需要发送帐户作为函数参数。
+ platon_sendTransaction
+ platon_sign(不安全且不建议使用)
+ platon_personalSign
+ platon_signTypedData

一旦您通过samurai连接到账户，可以随时通过检查platon.selectedAddress去检查当前账户。
...
交易是区块链中的常规操作。它们在Samurai中通过调用platon_sendTransaction方法去调用。它们可能涉及简单的lat/atp发送，可能是发送tokens，创建新的智能合约或以多种方式更改区块链上的状态。它们始终由外部帐户的签名或简单的密钥对启动。
...
在PlatON/Alaya中，每笔交易都有一个随机数。这样一来，每个交易只能由区块链处理一次。此外，要成为有效交易，随机数必须为0，或者已经处理了具有先前编号的交易。


这意味着始终按给定帐户的顺序处理交易，因此增加随机数是一个非常敏感的问题，很容易出现问题，尤其是当用户与多个应用程序通过同一帐户使用待处理交易进行交互时（可能跨多个帐户设备）。

由于这些原因，Samurai当前无法为应用程序开发人员提供任何自定义其建议的交易随机数的方法，而是帮助用户自己管理其交易队列。
...
在PlatON/Alaya中，pending交易池将其Gas price作为一种拍卖竞标价格提供给验证者，以将该交易包含在一个区块中以获取交易费。这意味着高昂的Gas price可能意味着更快的处理速度，但也意味着交易成本更高。


Samurai帮助用户在PlatON/Alaya主网络和流行的测试网络上选择具有竞争力的Gas price。我们向网络发出API请求，并允许用户在其Gas price的“慢”，“中”和“快速”选项之间进行选择。


我们无法了解所有区块链上的Gas Price，因为它需要进行深入分析。因此，尽管您可以放心地在我们的主要托管网络上忽略此参数，但是在您的应用程序比我们更了解目标网络的情况下，您可能会自己提供Gas price。
...
Gas limit是一个高度可选的参数，我们会自动为它计算一个合理的价格。您可能会了解某些原因下，您的智能合约会从自定义Gas limit中受益。
+ __To [semi-optional]__


Bech32编码的PlatON/Alaya地址。与收件人进行交易时必需（所有交易（合约创建除外））。
...
请注意，PlatON/Alaya中经常使用的这些数字比本地JavaScript数字具有更高的精度，并且如果无法预料，可能会导致不可预测的行为。因此，在处理打算用于区块链的值时, 我们强烈建议使用BN.js。
+ __Data [semi-optional]__


创建智能合约所需。

此字段还用于指定合约方法及其参数。您可以了解有关如何在Solidity ABI规范上对数据进行编码的更多信息。

+ __Chain ID [currently ignored]__


链ID当前由用户当前选择的网络（位于）得出。我们允许连接到多个网络，这时该参数将变得很重要，因此养成现在的习惯可能会很有用。
...
+ __事件(Events)__
  ...
  Samurai provider实现了Node.js EventEmitter API接口。本节详细介绍了通过该API发出的事件。在其他地方有很多EventEmitter相关的指南，但是您可以像如下这样监听事件：
  ```javascript
